...
//Initialisation et Instructions
    ...
    n_nbltr = strlen(s_mot); //On affecte une valeur à la variable de la longueur de notre mot à l'aide d'une commande qui va compter le nombre de caractères
...
//Jeu
    while ((n_vie > 0 && n_win != 1)) { //Boucle while du jeu, interrompue si l'une des deux conditions est fausse
        ...
        int n_nbaffiche = 0; //On initialise une variable qui va se voir affecter le nombre de lettres trouvées
...
Description:
Cette procédure affiche l'avancé de la vie du joueur grâce au dessin du pendu
...
Description:
Cette procédure affiche la victoire ou la défaite aux joueurs par un message
*************************************************************************************************************************************************/
...
void condition_victoire (){
    ...
    if(n_vie==0){ //Si le joueur n'a plus de vies, donc si la condition n'est plus vraie, on affiche la suite, à savoir la potence complète, accompagnée d'un message de Game Over
...
Description:
Cette procédure permet de vérifier si les lettres insérées par l'utilisateur se trouvent dans le mot caché. Celle qui ne sont pas présente dans le mot previennent le
...
Description:
Cette procédure permet de placer les lettres trouvées par l'utilisateur au bon endroit. Sinon,un tiret y est placé pour montrer l'erreur
*************************************************************************************************************************************************/

void affichage_lettre (){

    for (int i = 0; i < n_nbltr; i++) {
                if (n_mot[i]==1){ //Ici le 1 définit le "vrai" dans notre jeu. 1: On a trouvé le caractère, 0: On n'a pas trouvé le caractère.
                    printf("%c ", s_mot[i]); //On affiche la lettre trouvée à sa place
                    n_nbaffiche++; //On incrémente pour chaque lettre trouvée cette variable
                } else {
